...
do_backup () {
    if mountpoint -q ${MOUNTDIR}; then
        ...
        if [ -n "${opt_log}" ]; then
            rsync -aAXEv -x --del --stats --log-file ${LOG}  /boot/ ${MOUNTDIR}/boot/
            rsync -aAXEv -x --del --stats --log-file ${LOG} --exclude={'tmp/**','proc/**','run/**','sys/**','mnt/**','var/swap','home/pi/.cache/**'} / ${MOUNTDIR}/
        else
            rsync -aAXEv -x --del --stats /boot/ ${MOUNTDIR}/boot/
            rsync -aAXEv -x --del --stats --exclude={'tmp/**','proc/**','run/**','sys/**','mnt/**','var/swap','home/pi/.cache/**'} / ${MOUNTDIR}/
        fi
    else
