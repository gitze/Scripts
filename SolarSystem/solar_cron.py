...
class solarcron:
    def __init__(self, cron_table, name="DEFAULT"):
        self.name = name
        self.last_run = "NONE"
        self.cron_table = cron_table
        self.state_changed=None
        self.state_last=None
        #DO / TEST INITIAL SETUP
    
    def trigger(self, manualDate=None):
        dt = manualDate
        if dt is None:
            dt = datetime.now()
        minute, hour, dom, month, dow = self.cron_table.split(' ')
        weekday = dt.isoweekday()
        
        current_run=dt.strftime("%Y-%m-%d %H:%M")
        if (current_run == self.last_run):
            cron_table_result = False
        else:        
            cron_table_result = _parse_arg(minute, dt.minute) \
                and _parse_arg(hour, dt.hour) \
                and _parse_arg(dom, dt.day) \
                and _parse_arg(month, dt.month) \
                and _parse_arg(dow, 0 if weekday == 7 else weekday, True)

        if (self.state_last != cron_table_result):
            dt=dt.strftime("%Y-%m-%d %H:%M:%S")
            logger.info(f"Cron '{self.name}':{dt}: Result:{cron_table_result} LastRun:{self.last_run} Schedule:{self.cron_table}")
        self.state_last = cron_table_result

        if (cron_table_result):
            self.last_run = current_run
        return cron_table_result
